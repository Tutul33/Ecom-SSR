import { NgModule, Component, ElementRef, OnInit, AfterViewInit, AfterContentInit, AfterViewChecked, OnDestroy, Input, Output, Renderer2, EventEmitter,
    forwardRef, ViewChild, ChangeDetectorRef, TemplateRef, ContentChildren, QueryList, ContentChild, ChangeDetectionStrategy, ViewEncapsulation, Optional, 
    Inject,
...
import { NG_VALUE_ACCESSOR, ControlValueAccessor, NgModel, FormControlName } from '@angular/forms';
    ...
    template: `
        <div #container [ngClass]="{'p-multiselect p-component':true,
            ...
            <div *ngIf="overlayVisible" [ngClass]="['p-multiselect-panel p-component']" [@overlayAnimation]="{value: 'visible', params: {showTransitionParams: showTransitionOptions, hideTransitionParams: hideTransitionOptions}}" (@overlayAnimation.start)="onOverlayAnimationStart($event)"onOverlayAnimationEnd
                    ...
                    <ul class="p-multiselect-items p-component" [ngClass]="{'p-multiselect-virtualscroll': virtualScroll}" role="listbox" aria-multiselectable="true">
                        ...
                        <ng-template #itemslist let-optionsToDisplay let-selectedOption="selectedOption">
                            ...
                            <li *ngIf="hasFilter() && emptyOptions" class="p-multiselect-empty-message">
                                <ng-container *ngIf="!emptyFilterTemplate && !emptyTemplate; else emptyFilterTemplate || emptyTemplate">
                                    {{emptyFilterMessageLabel}}
                                </ng-container>
                            </li>
                            <li *ngIf="!hasFilter() && emptyOptions" class="p-multiselect-empty-message">
                                <ng-container *ngIf="!emptyTemplate; else emptyTemplate">
                                    {{emptyMessageLabel}}
                                </ng-container>
                            </li>
                        </ng-template>
                ...
                <div class="p-multiselect-footer" *ngIf="isFooterVisible">
                    <textarea
                        placeholder="Other text"
                        rows="2"
                        style="width: 100%"
                        (input)="addCustomItems($event)"
                        [value]="userText"
...
export class MultiSelect implements OnInit,AfterViewInit,AfterContentInit,AfterViewChecked,OnDestroy,ControlValueAccessor {
    ...
    constructor(public el: ElementRef, public renderer: Renderer2, public cd: ChangeDetectorRef, public filterService: FilterService, public config: PrimeNGConfig, public overlayService: OverlayService,@Inject(PLATFORM_ID) private platformId: Object
        ) {}
    ...
    writeValue(value: any) : void {
        // Added by Mozahedul Islam
        const values = value && value != null && value.length > 0 ? value.split(',').map((s: any) => {
            //var numbers = /^[0-9]+$/;
            const currentVal = s.trim();
            return currentVal;
            // if(currentVal.match(numbers))
        ...
        if(values.length){
            ...
            values.forEach((item:any) => {
                const ifItemFound = this.options.filter((x) => x[this.optionValue].toString() === item.toString());
                if (ifItemFound.length) {
                    valueList.push(item);
                }
                else{
                    userTextList.push(item);
                }
            });
            this.value = valueList;
            this.userText = userTextList.join(",");
        }
        else {
            this.userText = '';
            this.value = value;
        ...
        // this.value = value; // This was original
        this.updateLabel();
    ...
    updateFilledState() {
        // Modiefied by Mozahedul Islam
        this.filled = ((this.value && this.value.length > 0) || this.userText.length > 0);
        // this.filled = (this.value && this.value.length > 0); // Original line
    }
    ...
    onOptionClick(event) {
        ...
        const selectedValues = this.value.join(",");
        const modelValue = this.userText.length > 0 ? selectedValues + "," + this.userText : selectedValues;
        this.onModelChange(modelValue);

        //this.onModelChange(this.value); // original line
        //this.onChange.emit({originalEvent: event.originalEvent, value: this.value, itemValue: optionValue}); // original line

        this.onChange.emit({originalEvent: event.originalEvent, value: modelValue, itemValue: optionValue});
        this.updateLabel();
        this.updateFilledState();
    }
    ...
    findSelectionIndex(val: any): number {
        ...
        if (this.value) {
            for (let i = 0; i < this.value.length; i++) {
                if (ObjectUtils.equals(this.value[i].toString(), val.toString(), this.dataKey)) {
                    index = i;
                    break;
                }
            }
    ...
    toggleAll(event) {
        ...
        //this.onModelChange(this.value); // Original
        // this.onChange.emit({ originalEvent: event, value: this.value }); // original

        this.onChange.emit({ originalEvent: event, value: modelValue });
        this.updateFilledState();
        this.updateLabel();
    ...
    // Added by Mozahedul Islam for adding inserted text into textarea
    // Update all the model value and update filled state and emit onChange with inserted value
    addCustomItems(event: any){
        this.userText = event.currentTarget.value || '';
        const selectedValues = this.value != null ? this.value.join(",") : '';
        let modelValue = '';
        modelValue = selectedValues.length > 0 ? modelValue + selectedValues : modelValue;
        modelValue = this.userText.length > 0 ? modelValue.length > 0 ? modelValue + "," + this.userText : this.userText : modelValue;
        this.onModelChange(modelValue);
        this.onChange.emit({originalEvent: event.originalEvent, value: modelValue, itemValue: this.userText});
        this.updateLabel();
        this.updateFilledState();
    }
    ...
    updateLabel() {
        if (this.value && this.options && this.value.length && this.displaySelectedLabel) {
            ...
            // Modified by Mozahedul Islam
            // Added inserted text into textarea
            if (this.userText.length > 0) {
                label = label + ',' + this.userText;
            }
            // Modified by Mozahedul Islam
            ...
            if (this.userText.length > 0) {
                this.valuesAsString = this.userText;
            } 
            else {
    ...
    searchLabelByValue(val: any, options: any[]): string {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            // Change by zahid, Previous was only optionValue inside ObjectUtils.equals of second parameter
            if (val == null && optionValue == null || ObjectUtils.equals(val.toString(), optionValue.toString(), this.dataKey)) {
