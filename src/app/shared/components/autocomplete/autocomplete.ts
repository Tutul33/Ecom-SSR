    ...
    ControlValueAccessor
{
  ...
  filteredDataSource(query: any) {
    ...
    for (let i = 0; i < this.datasource.length; i++) {
      ...
      if(query.trim() != "")
      {
        if(item[this.field].toLowerCase().includes(query.trim().toLowerCase()))
        {
          filtered.push(item);
        }
      }
  ...
  resolveFieldData(value) {
    let data = this.field
      ...
      : value;
    return data !== (null || undefined) ? data : typeof value === 'string' ? value : "" ;
  }
