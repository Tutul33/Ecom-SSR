      ...
      if (!isNaN(Number(code))) {
        ...
        if (msg) {
          ...
          if (Number(msg.severity) === 1) {
            this.toastr.success(msg.engMessage, '', { timeOut: 2000 });
          } else if (Number(msg.severity) === 2) {
            this.toastr.info(msg.engMessage, '', { timeOut: 2000 });
          } else if (Number(msg.severity) === 4) {
            this.toastr.error(msg.engMessage, '', { timeOut: 2000 });
          } else {
            this.toastr.info(msg.engMessage, '', { timeOut: 2000 });
          }
        } else {
