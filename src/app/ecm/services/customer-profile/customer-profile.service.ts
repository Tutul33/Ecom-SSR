...
export class CustomerProfileDataService {
  ...
  getMemberPointHistoryList(spData : any, customerID: number): Observable<any> {
    return this.apiSvc
      .executeQuery(`${this.controllerName}/GetMemberPointHistoryList`, { data: JSON.stringify(spData), CustomerID: customerID })
      .pipe(
        map((response: any) => {
          return response.body;
        })
  ...
  getMemberDetailByID(customerID: number): Observable<any> {
    return this.apiSvc
      .executeQuery(`${this.controllerName}/GetMemberDetailByID`, { id: customerID })
      .pipe(
        map((response: any) => {
          return response.body;
        })
...
export class CustomerProfileModelService {
  ...
  prepareCustomerInfo(customerInfo: any) {
    try {
      ...
      this.customerInfo.imageFile = null;
      this.customerInfo.imageFile = new ImageFile();
      this.customerInfo.imageFile.folderName = Config.imageFolders.user;
      if (this.customerInfo.profilePicFileName) {
        this.customerInfo.imageFile.id = customerInfo.id;
  ...
  afterProfileSave(customerProfile: any) {
    try {
      ...
      GlobalConstants.customerInfo.profilePicFileName = GlobalMethods.jsonDeepCopy(this.customerInfo.profilePicFileName);
    } catch (e) {
      throw e;
    }
  }
